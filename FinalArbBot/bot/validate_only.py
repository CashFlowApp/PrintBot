...
def main() -> int:
    ...
    if markets:
        ...
        token_ids = list(dict.fromkeys(token_ids))[:20]
    if not token_ids:
        token_ids = ["dummy_token_for_ws_subscribe"]
    ...
    ws_market = PolymarketWSMarket(token_ids, on_market)
    ws_user = PolymarketWSUser(token_ids, on_user)
    ...
    logger.info("validate_only: startup validation complete. Exiting (no trading).")
    return 0

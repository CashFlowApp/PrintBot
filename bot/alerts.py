...
def alert_protection(message: str) -> None:
    """Log and send to Telegram (if enabled) for one-side protection triggers."""
    logger.info("[PROTECTION] {}", message)
    if _send_telegram(f"[PROTECTION] {message}"):
        logger.debug("Telegram sent: protection alert")

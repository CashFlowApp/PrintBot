...
def main() -> int:
    ...
    t_u: Optional[threading.Thread] = None
    if config.USER_WS_ENABLED:
        ws_user = PolymarketWSUser(all_token_ids or [], on_user_message)
        t_u = ws_user.run_forever_in_thread()
    else:
        logger.info("User WS disabled (USER_WS_ENABLED=false) — running without fill tracking")
    settlement.start()

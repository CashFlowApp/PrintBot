...
class ArbEngine:
    ...
    def check_all_pairs(self, pairs: list[dict]) -> None:
        """Loop over active pairs: for each, check YES ask + NO ask < 1.0 - MIN_EDGE_DYNAMIC and trigger if depth OK."""
        for p in pairs:
            market_id = p.get("market_id")
            yes_token = p.get("yes_token")
            no_token = p.get("no_token")
            if not market_id or not yes_token or not no_token:
                continue
            self.try_arb(market_id, yes_token, no_token)
            self.check_protection(market_id, yes_token, no_token)
